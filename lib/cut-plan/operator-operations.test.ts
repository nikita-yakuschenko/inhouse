...
describe("buildOperatorWorkflowSteps", () => {
  it("строит семь шагов для детали П(Ц)-1 [01] - 1", () => {
    ...
    expect(steps.map((step) => step.description)).toEqual([
      ...
      "Нанести маркировку на деталь П(Ц)-1 [01]",
      "Извлечь готовую деталь П(Ц)-1 [01]",
      "Разместить готовую деталь П(Ц)-1 [01] в месте складирования",
    ]);
  });
  ...
  it("строит десять шагов для детали П(Ц)-1 [06] - 1 с двумя резами", () => {
    ...
    expect(steps.map((step) => step.description)).toEqual([
      ...
      "Нанести маркировку на деталь П(Ц)-1 [06]",
      "Извлечь готовую деталь П(Ц)-1 [06]",
      "Разместить готовую деталь П(Ц)-1 [06] в месте складирования",
    ]);
  });
});
